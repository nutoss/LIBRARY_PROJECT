...
select BAA.id_book
	   ...
	   , (
	      isnull((select   sum(BS.Quantity) - isnull(sum(CI.Quantity),0)
			   from Bookstock as BS
			  left join Check_in_out_books as CI on BS.id_book = CI.id_book where CI.Date_in_Fact is null --or CI.Date_in_Fact < GETDATE())
			  and BS.id_Book = BAA.id_book
			 group by  BS.id_Book), (select   sum(BS.Quantity)
			   from Bookstock as BS
			  left join Check_in_out_books as CI on BS.id_book = CI.id_book where CI.Date_in_Fact is not null --or CI.Date_in_Fact < GETDATE())
			  and BS.id_Book = BAA.id_book
			 group by  BS.id_Book)
						)
